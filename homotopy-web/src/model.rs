...
use homotopy_dsl::{CompileOptions, Diagnostic, SymbolInfo};
...
pub enum Action {
    ...
    ImportProject(proof::SerializedData),
    ExportProof,
    ExportProject,
    ExportActions,
...
impl Default for State {
    fn default() -> Self {
        ...
        Self {
            ...
            active_preset: Some(preset.id.to_owned()),
        }
    }
}
    ...
    pub fn update(&mut self, action: Action) -> Result<bool, ModelError> {
        match action {
            ...
            Action::ImportProject(data) => {
                let project = read_project_bundle(&data.0)?;
                self.source = project.source;
                self.active_preset = project.active_preset;
                self.source_diagnostics.clear();
                self.source_symbols.clear();
                self.refresh_source_analysis();
                self.update(Action::Proof(proof::Action::ImportProof(
                    project.proof.into(),
                )))?;
            }
            ...
            Action::ApplySource => {
                ...
                self.source_symbols = result.symbols;
                if let Some(compiled_proof) = result.proof {
                    let data = serialize::serialize(
                        compiled_proof.signature,
                        compiled_proof.workspace,
                        compiled_proof.metadata,
                    );
                    self.update(Action::Proof(proof::Action::ImportProof(data.into())))?;
                }
            }
            Action::LoadPreset(id) => {
                let preset = presets::get(&id).ok_or(ModelError::UnknownPreset)?;
                self.source = preset.source.to_owned();
...
pub enum ModelError {
    ...
    #[error("project bundle could not be read")]
    Project,
...
fn project_bundle(proof: &[u8], source: &str, active_preset: Option<&str>) -> Vec<u8> {
    let manifest = ProjectManifest {
        ...
        didactic: Some("didactic.json".to_owned()),
    };
    let didactic = DidacticData {
        active_preset: active_preset.map(str::to_owned),
    };
    let manifest = serde_json::to_vec_pretty(&manifest).unwrap();
    let didactic = serde_json::to_vec_pretty(&didactic).unwrap();

    zip_files(
        [
            ("manifest.json".to_owned(), manifest),
            ("proof.hom".to_owned(), proof.to_vec()),
            ("source.homl".to_owned(), source.as_bytes().to_vec()),
            ("didactic.json".to_owned(), didactic),
        ]
        .into_iter(),
    )
}

fn read_project_bundle(data: &[u8]) -> Result<ProjectBundle, ModelError> {
    ...
    let proof = read_zip_bytes(&mut archive, &manifest.proof)?;
    let source = read_zip_string(&mut archive, &manifest.source)?;
    let active_preset = manifest
        .didactic
        .as_deref()
        .and_then(|name| read_zip_string(&mut archive, name).ok())
        .and_then(|json| serde_json::from_str::<DidacticData>(&json).ok())
        .and_then(|data| data.active_preset);
...
fn read_zip_string(
    ...
    String::from_utf8(read_zip_bytes(archive, name)?).map_err(|_| ModelError::Project)
}
...
mod tests {
    ...
    #[test]
    fn project_bundle_round_trips_source_proof_and_didactic_metadata() {
        let data = project_bundle(b"proof-bytes", "cell A;\nshow A;\n", Some("basics"));
        let bundle = read_project_bundle(&data).expect("project bundle should read");
        assert_eq!(bundle.proof, b"proof-bytes");
        assert_eq!(bundle.source, "cell A;\nshow A;\n");
        assert_eq!(bundle.active_preset.as_deref(), Some("basics"));
    }
}
