...
impl Component for ProjectView {
    ...
    fn view(&self, ctx: &Context<Self>) -> Html {
        let export_project = ctx
            .props()
            .dispatch
            .reform(|_| model::Action::ExportProject);
        let export_proof = ctx.props().dispatch.reform(|_| model::Action::ExportProof);
        ...
        html! {
            <>
                ...
                <button onclick={export_proof}>{"Export Proof"}</button>
                <label for="import" class="button">
                    {"Import"}
                </label>
                <input type="file" accept="application/msgpack,application/octet-stream,application/zip,.hom,.hio,.json,.zip" class="visually-hidden" id="import" onchange={import}/>
        ...
        match msg {
            Msg::ImportProof(file) => {
                let extension = std::path::Path::new(&file.name())
                    ...
                    .unwrap_or_default()
                    .to_owned();
                let is_project = extension.eq_ignore_ascii_case("hio");
                let is_zip = extension.eq_ignore_ascii_case("zip");
                let dispatch = dispatch.clone();
                let task = gloo::file::callbacks::read_as_bytes(&file.into(), move |res| {
                    let data = res.expect("failed to read file");
                    if is_project {
                        dispatch.emit(model::Action::ImportProject(data.into()));
                        return;
                    }
                    let serialized = is_zip
                        .then(|| {
                            ...
                            let mut archive =
                                zip::ZipArchive::new(std::io::Cursor::new(&data)).ok()?;
                        ...
                        .flatten()
                        .unwrap_or(data);
                    dispatch.emit(model::Action::Proof(model::proof::Action::ImportProof(
                        serialized.into(),
                    )));
                });
